...
public class LightRuleSet extends BaselineAwareRuleSupport implements SleepStageRule {
    ...
    public List<RuleHit> evaluate(AlignedSegmentContext context) {
        ...
        if (context.heartRate() >= lightHeartRateHigh) {
            hits.add(new RuleHit("LIGHT_004", "高心率抑制LIGHT规则", "心率偏高，更接近清醒或REM(" + source(BaselineMetricType.HEART_RATE) + ")", SleepStage.LIGHT, -0.8));
        }
