...
public class WakeRuleSet extends BaselineAwareRuleSupport implements SleepStageRule {
    ...
    public List<RuleHit> evaluate(AlignedSegmentContext context) {
        ...
        if (context.heartRate() >= wakeHeartRateHigh && context.respirationRate() >= RuleThresholds.RESP_WAKE_HIGH) {
            hits.add(new RuleHit("WAKE_009", "高心率高呼吸清醒规则", "心率和呼吸率均偏高，强烈提示清醒活动", SleepStage.WAKE, 2.0));
        }
